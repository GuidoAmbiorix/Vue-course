        ...
        <!-- <input
          ...
          :class="[
            'form-control',
            {
              ...
              // 'border-green-300': errors.title,
            },
          ]"
        /> -->
        <!-- <span class="text-red-500 capitalize" v-if="errors.title">{{ errors.title }}</span> -->
      </div>
    ...
    <div class="first-col">
      ...
      <div class="flex p-2 overflow-x-auto space-x-8 w-full h-[265px] bg-gray-200 rounded">
        <div v-for="image of images" :key="image.value" class="flex-shrink-0">
          <img :src="image.value" :alt="image.value" class="w-[250px] h-[250px ] rounded" />
        </div>
      ...
      <div class="my-4 text-right">
        <button
          type="submit"
          class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:bggray300"
          :disabled="isPending"
        >
          Guardar</button
